...
class Campus(models.Model):
    ...
    short_name = models.CharField(max_length=10, blank=True, null=True, verbose_name='簡稱')
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['is_active', 'sort_order'], name='campuses_active_sort_idx'),
        ]
...
class StudentNumberRule(models.Model):
    """學號規則 / Student Number Rule"""

    program_type = models.ForeignKey(
        ProgramType,
        on_delete=models.CASCADE,
        related_name='student_number_rules',
        verbose_name='學制'
    )
    department = models.ForeignKey(
        Department,
    ...
    admission_year_digits = models.PositiveSmallIntegerField(
        default=3,
        verbose_name='入學年位數'
    )
    note = models.CharField(max_length=255, blank=True, null=True, verbose_name='說明')
    is_active = models.BooleanField(default=True, verbose_name='是否啟用')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='建立時間')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新時間')
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['program_type', 'department'], name='snr_prog_dept_idx'),
        ]

    def __str__(self):
        return f"[{self.program_type.code}] {self.department.code} - {self.department.name_zh}"
